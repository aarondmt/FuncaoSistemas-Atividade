...
$(document).ready(function () {
    ...
    if (obj) {
        ...
        if (obj.ListaBeneficiario != undefined && obj.ListaBeneficiario.length > 0) {
            $.each(obj.ListaBeneficiario, function (i, v) {
                var linha = '<tr data-alterado="false" data-id="' + v.Id + '">' +
                    '<td>' + v.Cpf + '</td>' +
                    '<td>' + v.Nome + '</td>' +
                    '<td>' +
        ...
        if (validaCpf(cpf)) {
            if (VerificaExisteCpfNaTabela(cpf)) {
                ...
                $.ajax({
                    ...
                    success:
                        function (r) {
                            var linha = '<tr data-alterado="false" data-id="0">' +
                                '<td>' + cpf + '</td>' +
                                '<td>' + nome + '</td>' +
                                '<td>' +
                                botoesAlterarExcluirBeneficiario +
            ...
            ModalDialog("Ocorreu um erro", "O CPF informado não é um CPF válido!");
        }
    });
})
